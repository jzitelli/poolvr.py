...
def collide_balls(r_c,
    ...
    u_iR = v_i + array((R*omega_i[1], -R*omega_i[0], 0))
    u_jR = v_j + array((R*omega_j[1], -R*omega_j[0], 0))
    u_iR_mag = sqrt(dot(u_iR, u_iR))
    u_jR_mag = sqrt(dot(u_jR, u_jR))
    u_iC = v_i - array((R*omega_i[2], 0, -R*omega_i[0]))
    u_jC = v_j + array((R*omega_j[2], 0, -R*omega_j[0]))
    u_ijC = u_iC - u_jC
    u_ijC_xz = u_ijC[::2]
    ...
    while v_ij[1] < 0 or W < W_f:
        ...
        deltaOm_i = 5/(2*M*R) * array([( deltaP_2 + deltaP_iy),
                                       (-deltaP_ix),
                                       (-deltaP_1)])
        deltaOm_j = 5/(2*M*R) * array([( deltaP_2 + deltaP_jy),
                                       (-deltaP_jx),
                                       (-deltaP_1)])
        # update velocities:
        v_i0 = v_i
        v_i = v_i0 + array([deltaV_ix, deltaV_iy, 0])
        v_j0 = v_j
        v_j = v_j0 + array([deltaV_jx, deltaV_jy, 0])
